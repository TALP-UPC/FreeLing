...
\subsection{Install from \texttt{SVN} repositories}
...
\item \textbf{Install development tools}
 ...
 You'll need to install the C++ compiler, the GNU autotools, \textbf{plus} a SVN client.
\begin{verbatim}
...
\begin{verbatim}
 svn checkout http://devel.cpl.upc.edu/freeling/svn/trunk mysrc
\end{verbatim}
...
\begin{verbatim}
 svn checkout http://devel.cpl.upc.edu/freeling/svn/versions/freeling-3.0 mysrc
\end{verbatim}
\noindent (just replace \verb#freeling-3.0# with the desired version
number). You can see which versions are available by pointing your web
browser to \surl{http://devel.cpl.upc.edu/freeling/svn/versions}\footnote{Note
  that FreeLing-2.x versions require the appropiate \texttt{libomlet} and
  \texttt{libfries} versions (which you can checkout from the SVN in the
  same way).  Check the \emph{Download} section in FreeLing webpage to
  find out which \emph{Omlet\&Fries} version is suitable for each 2.x
  FreeLing version.}.
...
\end{verbatim}
 ...
 \verb#autoreconf --install# after \verb#svn update#.  You
