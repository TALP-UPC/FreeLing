...
#define MOD_TRACECODE MENTIONS_TRACE
  ...
  mention_detector::mention_detector(const wstring &filename, const wstring &lang) {

    // read configuration file and store information
    enum sections {LANGUAGE, LABELS, PROHIBITED_HEADS, REDUCE_MENTIONS, OTHER_SECTION};
    ...
    cfg.add_section(L"ProhibitedHeads",PROHIBITED_HEADS);
    labels_section[PROHIBITED_HEADS]=L"ProhibitedHeads";
    cfg.add_section(L"ReduceMentions",REDUCE_MENTIONS);
    labels_section[REDUCE_MENTIONS]=L"ReduceMentions";
	...
	exists_section[PROHIBITED_HEADS]=true;
        break;
      }
	...
	exists_section[REDUCE_MENTIONS]=true;
        break;
      }
      default: break;
      }
  ...
  ///////////////////////////////////////////////////////////////  
  map<wstring, freeling::regexp>& mention_detector::get_config_labels() {
    return _Labels;
  }
  ...
  void mention_detector::candidates(int sentn, paragraph::const_iterator s, int &wordn, sentence::const_iterator &itword, int &mentn, parse_tree::const_iterator pt, vector<mention> &mentions, subordinateType &stype) const {
  ...
  void mention_detector::add_mention(const mention &m, vector<mention> &mentions, int &n) const {
  ...
  void mention_detector::mark_initial_mentions_from(int i, vector<mention> &mentions) const{
  ...
  void mention_detector::discard_mentions(std::vector<mention> &mentions) const {
  ...
  void mention_detector::detect(const document &doc, vector<mention> &mentions) {
    TRACE(3,L"Searching for mentions");
    int sentn=0;
    int mentn=0;
    unsigned int first_mention_pos=0;

    TRACE(4,L"Mentions capacity = "+util::int2wstring((int) mentions.capacity()));
